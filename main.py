                            ...
                            try:
                                ...
                                if len(parts) >= 4:  # Minimum: /join team_code uid1 uid2 emote_id
                                    ...
                                    if len(uids) >= 1 and emote_id:
                                        # 1. Processing Msg
                                        processing_msg = f"[FFD700]🚀 Fast Join: {team_code}"
                                        ...
                                        # 5. Done Msg
                                        confirm_msg = f"[00FF00]✅ Fast Leave Done!"
...
async def perform_emote(team_code: str, uids: list, emote_id: int):
    ...
    try:
        # 1. Join the squad (Fast)
        EM = await GenJoinSquadsPacket(team_code, key, iv)
        await SEndPacKeT(None, online_writer, 'OnLine', EM)
        # Delay reduced from 1s to 0.2s
        await asyncio.sleep(0.2)
            
        # 2. Perform emote on each UID sequentially
        for uid_str in uids:
            if uid_str:  
                uid = int(uid_str)
                H = await Emote_k(uid, emote_id, key, iv, region)
                await SEndPacKeT(None, online_writer, 'OnLine', H)
                # Delay reduced from 0.5s to 0.1s
                await asyncio.sleep(0.1)

        # 3. LEAVE SQUAD (Updated Logic)
        try:
            leave_packet = await ExiT(None, key, iv)
        ...
        return {"status": "success", "message": "Emote performed & Left successfully!"}
    except Exception as e:
